...
export const AddNewBook = () => {
    ...
    const base64ConversionForImages = async (img: any) => {
        //if img exist
        if (img.target.files[0]) {
            //Do something here.
            console.log(img.target.files[0])
            getBase64(img.target.files[0]);
        }
    }

    //call filereader will read the file and convert it to base 64
    //and update the img state
    ...
    const submitNewBook = async () => {
        ...
        if (authState && authState.isAuthenticated && title !== '' && author !== '' && category !== "Category" && description !== '' && copies >= 0) {
            ...
            try {
                axios.post(url, JSON.stringify(book), requestOptions);
